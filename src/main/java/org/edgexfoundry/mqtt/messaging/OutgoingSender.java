...
public class OutgoingSender implements MqttCallback {
	...
	private final static Logger logger = Logger.getLogger(OutgoingSender.class);
	private MqttClient client = null;

	// needed in order to get acknowledgement
	private static final int outgoingMQTTQos = 0;
	private static final int outgoingMQTTKeepAlive = 3600;
	...
	public OutgoingSender(String broker, String clientId, String user, String password, String topic) {
		super();
		this.broker = broker;
	...
	public boolean sendMessage(byte[] messagePayload) {
		...
		if (client != null) {
			try {
				...
				message.setQos(outgoingMQTTQos);
				message.setRetained(false);
				client.publish(topic, message);
				return true;
			} catch (Exception e) {
				logger.error("Failed to send outbound message (unexpected issue): " + new String(messagePayload));
	...
	private void connectClient() {
		try {
			...
			connOpts.setKeepAliveInterval(outgoingMQTTKeepAlive);
			logger.debug("Connecting to broker:  " + broker);
			client.connect(connOpts);
			logger.debug("Connected");
		} catch (MqttException e) {
			logger.error("Failed to connect to MQTT client ( " + broker + "/" + clientId + ") for outbound messages");
			logger.error(e.getLocalizedMessage());
			e.printStackTrace();
		}
	}
