...
public class DeviceStore {
	...
	// cache for devices
	private Map<String, MQTTDevice> devices = new HashMap<String,MQTTDevice>();
	...
	public boolean remove(String deviceId) {
		MQTTDevice d = devices.values().stream().filter(device -> device.getId().equals(deviceId)).findAny().orElse(null);
		if (d != null) {
			remove(d);
		}
		return true;
	...
	private Device addDeviceToMetaData(Device device) {
		...
		try {
			...
			try {
				...
				logger.error("Could not add new device " + device.getName() + " to metadata with error " + e.getMessage());
				return null;
			}
		...
		devices.put(device.getName(), MQTTDevice);
		return device;
	}
	...
	public Map<String, Device> getDevices() {
		Map<String, Device> d = devices.entrySet().stream().collect(Collectors.toMap(
				e -> e.getKey(),
				e -> e.getValue().getDevice()));
		return d;
	}
	
	public Map<String, MQTTDevice> getMQTTDevices() {
		return devices;
	}
	...
	public Device getMetaDevice(String deviceName) {
		...
		Device result = metaDevices.stream().filter(device -> deviceName.equals(device.getName())).findAny().orElse(null);
		return result;
	}
	
	public Device getMetaDeviceById(String deviceId) {
		List<Device> metaDevices = getMetaDevices();
		Device result = metaDevices.stream().filter(device -> deviceId.equals(device.getId())).findAny().orElse(null);
		return result;
	}
	...
	public MQTTDevice getMQTTDeviceById(String deviceId) {
		if (devices != null) {
			return devices.values().stream().filter(
					device -> 
					device.getId().equals(deviceId)).findAny().orElse(null);
		}
		return null;
	}
