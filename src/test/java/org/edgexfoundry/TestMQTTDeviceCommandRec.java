...
/**
 * Use this class to test receiving commands from the device service and responded back for get commands.  Use a REST client to send a command to the service like:
 * http://localhost:49982/api/v1/devices/<command id>/message  - use POST on this one with {"message":"some text"} in body
 * http://localhost:49982/api/v1/devices/<command id>/ping - use GET
 * http://localhost:49982/api/v1/devices/<command id>/randnum - use GET
 * 
...
public class TestMQTTDeviceCommandRec implements MqttCallback {
	...
	private static String REQ_MQTT_USER = "vyrnnobp";
	private static String REQ_MQTT_PASS = "q4u83TLxjCaI";
	...
	private static String RESP_MQTT_USER = "vyrnnobp";
	private static String RESP_MQTT_PASS = "q4u83TLxjCaI";
	...
	private void sendResponse(String returnPayload) {
		...
		OutgoingSender sender = new OutgoingSender(address, RESP_MQTT_CLIENT_ID, RESP_MQTT_USER, RESP_MQTT_PASS,
				RESP_MQTT_TOPIC);
